...
export function buildApp() {
  ...
  // Swagger UI Setup (Non-production only)
  if (!isProd) {
    try {
      const specPath = path.resolve(__dirname, '../docs/openapi.yaml');
      const specFile = fs.readFileSync(specPath, 'utf8');
  ...
  /**
   * Option 2: Maintain CSP in all environments but add Swagger UI exceptions.
   * This ensures integration tests (NODE_ENV=test) pass while dev remains usable.
   */
  app.use(helmet({
    contentSecurityPolicy: {
      directives: {
        defaultSrc:     ["'none'"],
        // 'unsafe-inline' is required for Swagger UI to execute its internal scripts
  ...
  // Apply express.json() to every route EXCEPT the payment webhook endpoint.
  app.use((req, res, next) => {
    if (req.path === '/vans/webhook/payment') {
      next();
      return;
